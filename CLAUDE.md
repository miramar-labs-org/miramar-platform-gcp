...
| Build MLMD arm64 Images  | `build-mlmd-arm64.yaml`         | Build arm64 images for the KFP MLMD stack (ml_metadata_store_server + kfp-metadata-writer) natively on the DGX using Bazel. Both jobs run in parallel (~45-60 min each, cached on repeat). Run before first Kubeflow Deploy or after a KFP version bump. See `dgx/minikube/kubeflow/arm64/README.md`.                                                                                                                                                                                                                                                                                                                                                     |
| Kubeflow Deploy          | `deploy-kubeflow.yaml`          | Deploy Kubeflow Pipelines standalone into the kubeflow namespace via `kubectl apply -k`; patches MLMD stack to arm64-compatible images (deploykf + miramar-labs-org builds); UI at port 8080 via kubeflow-portfwd.service. Prerequisite: Build KFP arm64 Images. Input: `pipeline_version` (default `2.16.1`).                                                                                                                                                                                                                                                                                                                                             |
