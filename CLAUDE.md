...
| `wsl2/bootstrap.sh` | One-time setup for a fresh WSL2 distro — installs dev tools, Docker, Kubernetes tooling, pyenv, Miniforge, Go, Java, and configures SSH (sshd on port 2222, ed25519 key, mDNS, `~/.ssh/config` for lab hosts). Also generates `id_ed25519_smb` keypair and bakes `~/.smbcredentials` into the distro. Run inside the distro before exporting the template; copy the printed `id_ed25519_smb.pub` to `wsl2/id_ed25519_smb.pub` in the repo. |
| `wsl2/post-provision.ps1` | PowerShell script run on the Windows host by the **WSL2 Post-Provision** workflow. Params: `-Name` (distro), `-User`, `-Port` (default 2222), `-DgxHost` (default `spark-79b7.local`). No `DGX_SMB_PASSWORD` needed — credentials are baked into the template by `bootstrap.sh`. Handles: `.wslconfig` (mirrored networking), per-distro firewall rule (`WSL2 SSH <Port> Inbound`), sshd port override inside the distro, writes `/etc/wsl2-distro-name`, triggers `wsl2-ssh-setup.service` (smbclient syncs SSH files, adds pubkey, adds `wsl2-<Name>` host block to shared config). |
| `wsl2/setup-shared-ssh.sh` | Installed at `/usr/local/bin/setup-shared-ssh.sh` on WSL2 distros (by `bootstrap.sh`) and on Orin (by **Setup Shared SSH Store**). Uses smbclient to sync SSH files from `DGX ~/shared/ssh/` — no CIFS mount required. Called by `wsl2-ssh-setup.service` (WSL2) and `orin-ssh-setup.service` (Orin) on every cold start. |
...
| Setup Shared SSH Store | `setup-shared-ssh.yaml` | **One-time setup** (re-run after each template rebuild). Initialises `~/shared/ssh/` on DGX, creates `~/.ssh` symlinks on DGX, pre-authorizes `wsl2/id_ed25519_smb.pub` (template SMB key) on DGX, wires Orin via `orin-ssh-setup.service` (smbclient — no CIFS). Orin setup SSHes to `localhost:22` from the agx runner container (container uses `--network=host`). Requires vars `DGX_HOST`, `DGX_HOST_USER` and secrets `DGX_HOST_SSH_KEY`, `ORIN_HOST_SSH_KEY`, `DGX_SMB_PASSWORD`. **Orin prerequisite:** restore any `.bak` SSH files and self-authorize `id_ed25519.pub` before first run — see README. |
