...
| `wsl2/bootstrap.sh` | One-time setup for a fresh WSL2 distro — installs dev tools, Docker, Kubernetes tooling, pyenv, Miniforge, Go, and configures SSH (sshd on port 2222, ed25519 key, mDNS, `~/.ssh/config` for lab hosts). Requires `DGX_SMB_PASSWORD` (baked into `~/.smbcredentials`) and `DGX_PUBKEY` (Spark's pubkey seeded into `authorized_keys` so `DGX_HOST_SSH_KEY` can SSH into fresh distros). Also generates `id_ed25519_smb` keypair and installs `setup-shared-ssh.sh`. Run inside the distro before exporting the template; copy the printed `id_ed25519_smb.pub` to `wsl2/id_ed25519_smb.pub` in the repo. |
| `wsl2/post-provision.ps1` | PowerShell helper for manual Windows-side setup. Params: `-Name` (distro), `-User`, `-Port` (default 2222), `-DgxHost` (default `spark-79b7.local`). Handles `.wslconfig` (mirrored networking), per-distro firewall rule, and sshd port override. Then calls `setup-shared-ssh.sh` inside the distro to mount `//DGX/shared` via CIFS and symlink `~/.ssh/`. Not used by GHA workflows (provision-wsl2.yaml SSHes directly into the distro). |
| `wsl2/setup-shared-ssh.sh` | Installed at `/usr/local/bin/setup-shared-ssh.sh` on WSL2 distros (by `bootstrap.sh`) and on Orin (by **Setup Shared SSH Store**). Mounts `//DGX/shared` at `~/shared/` via CIFS (`cifs-utils`), adds an fstab entry, then symlinks all `~/.ssh/` files (`config`, `known_hosts`, `authorized_keys`, `id_ed25519`, `id_ed25519.pub`) to `~/shared/ssh/`. Also writes the `wsl2-<name>` host block directly into the shared `config`. All machines share Spark's SSH identity — no per-machine keypair. Idempotent. |
| `wsl2/id_ed25519_smb.pub` | Template-level Samba bootstrap public key — generated by `bootstrap.sh`, committed to the repo after each template rebuild. Read by **Setup Shared SSH Store** to pre-authorize new distros on DGX before provisioning. |
