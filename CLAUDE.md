...
| Deploy Platform Dashboard | `deploy-dashboard.yaml` | Build and deploy the GitHub Pages project dashboard. Three status bars: DGX Spark, AGX Orin, GCP (GKE cluster, Zone, Node type, CPU pool node count, GPU pool badge, State bucket, GAR). Service columns show green/red active-state badges driven by org-level `{MACHINE}_{SERVICE}_ACTIVE` variables. Project table includes a 🗑 delete column — clicking opens a confirmation modal that fires `delete-project.yaml` via the GitHub API using a PAT stored in `localStorage` (prompted once, needs `delete_repo` + `workflow` scope). Runs hourly + on `workflow_run` completion of any state-writing workflow. URL: https://miramar-labs-org.github.io/miramar-platform-gcp/ |
