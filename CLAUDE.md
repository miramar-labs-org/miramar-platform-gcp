...
- **NVIDIA Jetson AGX Orin 64GB** — Ubuntu JetPack 6.x, aarch64 (arm64), 12-core Cortex-A78AE, Ampere GPU 2048 CUDA cores (sm_87), CUDA 12.6. Runner label: `orin`. Uses `mlabs-runner:jetson` (L4T-based) — Tegra unified-memory GPU is incompatible with standard `nvidia/cuda` server images.
