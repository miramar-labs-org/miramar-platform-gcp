...
| Postgres Deploy      | `deploy-postgres.yaml`      | Deploy Postgres into k3s `postgres-system` namespace; optionally provisions a consumer database/role and publishes the resulting `DATABASE_URL` to the run's step summary; writes `{MACHINE}_POSTGRES_ACTIVE` org var. Inputs: `runner`, `consumer_db`, `consumer_user`                                                       |
| Postgres Undeploy    | `undeploy-postgres.yaml`    | Remove Postgres Deployment/Service/PVC (namespace deletion optional -- the PV survives via Retain policy either way); clears `{MACHINE}_POSTGRES_ACTIVE` org var. Inputs: `runner`, `delete_namespace`                                                                                                                          |
| Model Router Deploy  | `deploy-model-router.yaml`  | Deploy LiteLLM proxy into `model-router` namespace on dgx/agx (k3s) or gke; routing table from `{machine}/k3s/model-router/litellm-config.yaml` or `gke/model-router/litellm-config.yaml`, plus (dgx) both hosts' Ollama catalogs read live at deploy time by `scripts/gha/gen-ollama-upstreams.py` as `dgx/*` and `agx/*`; sets `{MACHINE}_OPENWEBUI_API_URL` (dgx/agx) or `GKE_MODEL_ROUTER_ACTIVE` (gke). Inputs: `runner` (dgx \| agx \| gke) |
| Model Router Undeploy | `undeploy-model-router.yaml` | Remove `model-router` namespace; clears `{MACHINE}_OPENWEBUI_API_URL` (dgx/agx) or `GKE_MODEL_ROUTER_ACTIVE` (gke); triggers Open WebUI redeploy if active on dgx/agx. Inputs: `runner` (dgx \| agx \| gke)                                    |
| GKE Gateway Deploy   | `deploy-gke-gateway.yaml`   | Apply Gateway + HTTPRoute on GKE; exposes `https://api.miramar-labs.com/v1` → GKE model router; sets `GKE_GATEWAY_ACTIVE` + `GKE_GATEWAY_URL`. Also callable via `workflow_call` from GKE serving deploy workflows. No inputs.                     |
| GKE Gateway Undeploy | `undeploy-gke-gateway.yaml` | Delete Gateway + HTTPRoute; stops GCP LB billing (~$0.025/hr); retains static IP + SSL cert for instant redeploy; clears `GKE_GATEWAY_ACTIVE` + `GKE_GATEWAY_URL`. Also callable via `workflow_call`. No inputs.                                  |
...
| Nsight Operator Deploy      | `deploy-nsight-operator.yaml`      | Install NVIDIA Nsight Operator via Helm on DGX/AGX; reads `nsight-injector.privileged` from the per-host values file and, **only when it is `true`** (AGX, `RmProfilingAdminOnly=1`), deploys the `nsight-ape-webhook` mutating webhook + relaxes kubeflow PSA `enforce`→`privileged` (`relax_kubeflow_psa`, default true) so KFP step pods can run `privileged` for CUPTI collection. Where it is `false` (DGX) both are skipped and any leftovers are removed (**Remove privileged-only workarounds**). A preflight hard-fails when the host reports `RmProfilingAdminOnly: 1` while the values file says `privileged: false` — that mismatch otherwise yields reports with no GPU records; reload + restart `nsight-portfwd.service` (now forwards the coordinator REST API on `:13001` alongside the SPA on `:8889`) and curl-check `localhost:13001/api/v1/sessions/`; writes `{MACHINE}_NSIGHT_OPERATOR_ACTIVE`. Inputs: `runner`, `relax_kubeflow_psa`, `ape_webhook_tag` |
...
| Kubeflow Deploy        | `deploy-kubeflow.yaml`   | Deploy KFP standalone with native arm64 images; creates the `hf-model-cache` (200 Gi) PV + PVC in the `kubeflow` namespace backed by a k3s hostPath PV; writes `{MACHINE}_KFP_ACTIVE` org var. Inputs: `runner`. **Note:** also deletes the legacy `nsight-reports` PV/PVC if present — nothing mounts it (the Nsight Operator writes to its own MinIO and `scripts/nsight/export-report.sh` writes the host archive directly), and its hostPath exposed the profiling archive root `~/shared/nsight` as `ReadWriteMany`. |
...
| `serving-triton-vllm` | Triton Inference Server + vLLM Python backend on DGX/GKE (no AGX — `vllm-python-py3` not validated for Orin arm64): `serving-config.yaml` (base model, LoRA adapter ft_project, Triton image tag, vLLM flags), `model_repo/model/config.pbtxt` (Triton Python backend, fixed `name: "model"`), `model_repo/model/1/model.py` (AsyncLLMEngine in background event loop thread; LoRARequest from `/adapter/`), `Dockerfile.serve`, `k8s/triton.yaml` (GKE, emptyDir HF cache), `k8s/triton-k3s.yaml` (DGX K3s, GHCR image, hostPath), `build-push.yaml` / `deploy.yaml` / `undeploy.yaml`. Health: `/v2/health/ready`. LiteLLM: `model: triton/model`, no `/v1` suffix. | — |
| `serving-triton-trtllm` | Triton Inference Server + TRT-LLM Python backend on DGX/GKE (no AGX): `serving-config.yaml` (compression project + run_id, no adapter), `model_repo/model/1/model.py` (synchronous TRT-LLM `LLM` API — no threading), `Dockerfile.serve` (bakes GPU-arch engine: `engine_l4/` for GKE wsl2, `engine_gb10/` for DGX), `k8s/triton.yaml` / `k8s/triton-k3s.yaml`, `build-push.yaml` / `deploy.yaml` / `undeploy.yaml`. Starts in ~30s (no HF download). VRAM tracked via `DGX_TRITON_VRAM_GB`. | — |
...
| `kfp-eval`         | KFP v2 model bakeoff: `load_dataset` → per candidate (`serve_model` → task harnesses → `teardown_model`) → `judge_and_score` → `report`. Ranks N candidate LLMs × M serving modes (`ollama` = shared Ollama evict/preload; `guided` = transient vLLM Deployment, xgrammar + fp8) against a MinIO-frozen eval set, scored with deterministic per-task gates + a fixed 1–5 LLM-as-judge and a weighted composite. Notebook-driven build (`kfp_step` / `kfp_pipeline` tagged cells → `pipeline.py`); `matrix = models × serving_modes` unrolled serially at build time. `evallib/{scoring,rubric}.py` are pure + unit-tested and `# inline:`-spliced into `judge_and_score`. `scripts/export_dataset.py` freezes a LangSmith trace slice → MinIO. Winner logged to MLflow + `runs/RUNS.md`. `example_harness` ships as the reference — replace with real tasks. `guided` mode needs `kubectl apply -f manifests/bakeoff-rbac.yaml` once. `deploy-to-kfp.yaml` / `undeploy-from-kfp.yaml` workflows + CI badges. Topic tag `miramar-kfp-eval`. | `kfp>=2.0.0 kfp-kubernetes openai langsmith boto3 mlflow pandas pyyaml` |
...
Per-project workflows in every `serving-triton-vllm` repo:

| Workflow       | File              | Runner                  | Purpose                                                                                                                                                                                    |
| -------------- | ----------------- | ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Build and Push | `build-push.yaml` | `wsl2` (GKE) / `dgx`   | Scan ft-eval project for gate-passing adapter, build `Dockerfile.serve` (adapter + model_repo), push to GAR (GKE, amd64) or GHCR (DGX). No AGX build target.                              |
| Deploy         | `deploy.yaml`     | `ubuntu-latest` / `dgx` | Expand L4 spot pool (GKE) or unserve Ollama (DGX), apply `k8s/triton.yaml` / `triton-k3s.yaml`, wait rollout, smoke test `/v2/models/model/generate`, register `triton/model` in router |
| Undeploy       | `undeploy.yaml`   | `ubuntu-latest` / `dgx` | Deregister from model router BEFORE deleting namespace; GKE also triggers `gke-restore-gpu.yaml`; clears `DGX_SERVING_ACTIVE` / `GKE_SERVING_ACTIVE`                                      |

Per-project workflows in every `serving-triton-trtllm` repo:

| Workflow       | File              | Runner                  | Purpose                                                                                                                                                                                    |
| -------------- | ----------------- | ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Build and Push | `build-push.yaml` | `wsl2` (GKE) / `dgx`   | Locate GPU-arch engine (`engine_l4/` for GKE, `engine_gb10/` for DGX) in `~/shared/huggingface-kfp/engines/`, stage into build context, build `Dockerfile.serve`, push to GAR or GHCR    |
| Deploy         | `deploy.yaml`     | `ubuntu-latest` / `dgx` | Expand L4 spot pool (GKE) or unserve Ollama (DGX), apply Triton manifests, wait rollout (~30s cold start), smoke test, register `triton/model` in router; tracks `DGX_TRITON_VRAM_GB`    |
| Undeploy       | `undeploy.yaml`   | `ubuntu-latest` / `dgx` | Deregister from model router BEFORE deleting namespace; GKE also triggers `gke-restore-gpu.yaml`; clears `DGX_TRITON_VRAM_GB=0`                                                           |
