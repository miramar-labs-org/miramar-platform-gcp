...
| Model Router Deploy  | `deploy-model-router.yaml`  | Deploy LiteLLM proxy into k3s `model-router` namespace; routing table read from `dgx/k3s/model-router/litellm-config.yaml`; sets `{MACHINE}_OPENWEBUI_API_URL` to router URL; triggers Open WebUI redeploy if active. Inputs: `runner`               |
| Model Router Undeploy | `undeploy-model-router.yaml` | Remove `model-router` namespace; clears `{MACHINE}_OPENWEBUI_API_URL`; triggers Open WebUI redeploy if active (reverts to Ollama-only). Inputs: `runner`                                                                                            |
