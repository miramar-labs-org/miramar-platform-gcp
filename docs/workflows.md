...
| Model Router Deploy  | `deploy-model-router.yaml`  | Deploy LiteLLM proxy into `model-router` namespace on dgx/agx (k3s) or gke; routing table from `{machine}/k3s/model-router/litellm-config.yaml` or `gke/model-router/litellm-config.yaml`; sets `{MACHINE}_OPENWEBUI_API_URL` (dgx/agx) or `GKE_MODEL_ROUTER_ACTIVE` (gke). Inputs: `runner` (dgx \| agx \| gke) |
