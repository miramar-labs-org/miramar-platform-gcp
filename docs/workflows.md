...
| `serving-llm-nim`  | Local/fine-tuned model serving on DGX via NVIDIA Multi-LLM NIM (`nvcr.io/nim/nvidia/llm-nim`): `serving-config.yaml` (model host path, container name, served name, image tag), `k8s/nim-local-k3s.yaml` (K3s, model hostPath + nim-cache hostPath mounts, NGC_API_KEY secret), `deploy.yaml` / `undeploy.yaml` workflows + CI badges, `scripts/list-profiles.sh` (pre-deploy compatibility check), `smoke_test_prompts.jsonl`. DGX-only — no GKE target (model lives at a local host path). | —                                          |
...
| Workflow | File             | Runner | Purpose                                                                                                                                     |
| -------- | ---------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------- |
| Deploy   | `deploy.yaml`    | `dgx`  | Verify model path exists, run `list-model-profiles`, unserve Ollama, deploy `nvcr.io/nim/nvidia/llm-nim` to K3s, smoke tests, wire Open WebUI |
| Undeploy | `undeploy.yaml`  | `dgx`  | Delete `nim` deployment + service, reset Open WebUI backend to Ollama                                                                       |
