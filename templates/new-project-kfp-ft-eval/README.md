...
**Output** — per profiled stage on the host:
```
~/shared/nsight/{{PROJECT_NAME}}/{run_name}/
  baseline-eval/
    profile.nsys-rep    # Nsight Systems report
    profile.sqlite      # auto-generated event database (safe to delete)
    summaries.csv       # generated by /nsight-interpret
    analysis-claude.md  # LLM bottleneck analysis
```

**Interpreting results:**
```bash
/nsight-interpret run-NNN          # runs nsys stats + sends to Claude for analysis
nsys-ui ~/shared/nsight/.../profile.nsys-rep   # open in desktop GUI
```

**Cleaning up large artifacts:**
```bash
python3 scripts/purge_nsight.py --dry-run       # preview
python3 scripts/purge_nsight.py                 # delete .sqlite + .nsys-rep where summaries.csv exists
python3 scripts/purge_nsight.py --sqlite-only   # keep .nsys-rep, remove .sqlite only
```

See [kfp-skills.md — Nsight Profiling in KFP](https://github.com/miramar-labs-org/miramar-platform-gcp/blob/main/docs/kfp-skills.md#nsight-profiling-in-kfp) for full details on nsys flags, privileged pod patch, and GB10 unified memory notes.
