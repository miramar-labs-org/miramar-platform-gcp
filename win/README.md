...
| Nsight coordinator   | `13001`         | `13002`         | `13001`     |
...
update the `.tlp` files automatically. The Postgres and Nsight coordinator rows above (and
the pre-existing Nsight UI / Open WebUI gap) still need the forwarding rule added manually
in the Bitvise client (**Profile settings → New port forwarding rule**), then re-saved.
